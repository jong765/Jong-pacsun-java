...
public class FileProcess
{
  ...
  protected void validateXml(File xmlFile) throws InvalidXmlException {
    this.schemaValidator.setSchemaName(this.XML_SCHEMA_FILE);
    this.schemaValidator.setXmlFile(xmlFile);
    try {
      this.schemaValidator.validate();
      if (!this.schemaValidator.isValidSchema())
    ...
    while (retryMoveFile)
      if (!FileUtils.moveFile(fromFullFileName, toFullFileName)) {
        if (retrialCount < Integer.parseInt(this.RETRIAL_ATTEMPT)) {
          retrialCount++;
          errorMessage = getClass().getName() + " : " + "Error in moving file " + xmlFileName + " from " + sourceFolder + 
            " to " + targetFolder;
  ...
  {
    return FileUtils.listFiles(folderPath, filePrefix, fileExtension.toLowerCase());
  }
 ...
 * Qualified Name:     com.pacsun.hingeto.process.FileProcess
